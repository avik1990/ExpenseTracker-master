...
public class YearlyExpenseAdapter extends RecyclerView.Adapter<YearlyExpenseAdapter.MyViewHolder> {

    private List<Months> countryList;
    Context mContext;
    GetYearFromreturnedYearView getMonthFromAdapter;
    String currentmonth = "";
    ...
    public interface GetYearFromreturnedYearView {
        String returnedYearView(String month);
    }
    ...
    public YearlyExpenseAdapter(Context mContext, List<Months> countryList, GetYearFromreturnedYearView getMonthFromAdapter, String currentmonth, List<ExcelDataModel> listPopup) {
        ...
        if (listPopup.size() > 0) {
            for (int i = 0; i < listPopup.size(); i++) {
                listmonth.add(listPopup.get(i).getDate());
                for (int j = 0; j < listPopup.get(i).getGroupFocAll().size(); j++) {
                    if (!listPopup.get(i).getGroupFocAll().get(j).getDate().isEmpty()) {
                       /* String m[] = listPopup.get(i).getGroupFocAll().get(j).getDate().split("-");
                        String month = Utils.getCurrentMonths(m[1]);*/
    ...
    public void onBindViewHolder(final MyViewHolder holder, int position) {
        ...
        holder.tv_checfname.setText(c.getYearName());

        if (listmonth.size() > 0) {
            if (listmonth.contains(c.getYearName())) {
                int pos = listmonth.indexOf(c.getYearName());
        ...
        holder.rl_mainview.setOnClickListener(v -> getMonthFromAdapter.returnedYearView(holder.tv_checfname.getText().toString()));
    }
