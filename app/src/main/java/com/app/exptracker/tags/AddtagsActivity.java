package com.app.exptracker.tags;
...
public class AddtagsActivity extends BaseActivity implements View.OnClickListener, TagAdapter.GetUserIDAdapter {
    ...
    public void onClick(View v) {
        if (v == fab_add) {
            ...
            if (et_tag.getText().toString().trim().isEmpty()) {
                Utils.ShowToast(context, "Add Label");
                return;
            }

            ExcelDataModel excelDataModel = new ExcelDataModel();
            excelDataModel.setTagName(et_tag.getText().toString().trim());

            returnedId = db.addLabels(excelDataModel);

            if (returnedId < 0) {
                Utils.ShowToast(context, "Label Already Exists");
            } else {
                // finish();
                et_tag.setText("");
                fetchUserData();
                Utils.ShowToast(context, "Label Added");
    ...
    public void fetchUserData() {
        listUser.clear();
        listUser = db.getALlTags();
        if (listUser.size() > 0) {
            rl_recyclerview.setVisibility(View.VISIBLE);
            userAdapter = new TagAdapter(context, listUser, this);
    ...
    public void returnPositionImageView(int pos, CircleImageView iv_imgview) {
        ...
        tagID = listUser.get(pos).getTagId();
        ...
        if (requestCode == REQUEST_IMAGE) {
            /*if (resultCode == Activity.RESULT_OK) {
                ...
                try {
                    ...
                    db.updateUserImage(tagID, Utils.convert(bitmap));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }*/
    ...
    private void launchCameraIntent() {
        Intent intent = new Intent(AddtagsActivity.this, ImagePickerActivity.class);
    ...
    private void showSettingsDialog() {
        AlertDialog.Builder builder = new AlertDialog.Builder(AddtagsActivity.this);
